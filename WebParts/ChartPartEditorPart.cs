...
namespace ChartPart {
    public class ChartPartEditorPart : BaseEditorPart {
        ...
        public ChartPartEditorPart():base(false) {
            this.Title = Localization.Translate("ChartPartSettings");
        ...
        protected override void FillEditorPanel() {
            ...
            m_chartType.Items.Add(SeriesChartType.Bubble.ToString());
            
            m_chartType.Items.Add(SeriesChartType.Pie.ToString());
            m_chartType.Items.Add(SeriesChartType.Doughnut.ToString());
            m_chartType.Items.Add(SeriesChartType.Radar.ToString());
            
            m_chartType.Items.Add(SeriesChartType.Funnel.ToString());
            m_chartType.Items.Add(SeriesChartType.Pyramid.ToString());


            m_title = CreateEditorPartTextBox();
            AddToolPaneRowWithBuilder(CreateToolPaneRow(Localization.Translate("Title"), new Control[] { m_title }), m_title);
            AddToolPaneRowWithBuilder(CreateToolPaneRow(Localization.Translate("Site"), new Control[]{m_siteUrl}), m_siteUrl);
            AddToolPaneRow(CreateToolPaneRow(Localization.Translate("List"), new Control[] { m_list }));
            AddToolPaneRow(CreateToolPaneRow(Localization.Translate("View"), new Control[] { m_view }));
            AddToolPaneRow(CreateToolPaneSeparator());
            AddToolPaneRow(CreateToolPaneRow(Localization.Translate("XSeries"), new Control[] { m_xcols }));
           // AddToolPaneRow(CreateToolPaneRow("Secondary series", new Control[] { m_xcols2 }));
            AddToolPaneRow(CreateToolPaneSeparator());
            AddToolPaneRow(CreateToolPaneRow(Localization.Translate("YSeries"), new Control[] { m_ycols }));

            AddToolPaneRow(CreateToolPaneSeparator());
            AddToolPaneRow(CreateToolPaneRow(Localization.Translate("ChartType"), new Control[] { m_chartType }));
        ...
        public override bool ApplyChanges() {
            ...
            if (chartPart != null) {
                ...
                chartPart.X2AxisSourceColumns = new List<string>();
                foreach (ListItem li in m_xcols2.Items) {
                    if (li.Selected) {
                        chartPart.X2AxisSourceColumns.Add(li.Value);
                    }
                }
            ...
            foreach (SPField field in sellist.Fields) {
                if (selview.ViewFields.Exists(field.InternalName)) {
                    if (isNumericField(field) || field.Type == SPFieldType.Calculated) {
                        ...
                        m_xcols2.Items.Add(new ListItem(field.Title, field.InternalName));
                    }
                    m_ycols.Items.Add(new ListItem(field.Title, field.InternalName));                    
                }
            }
            m_xcols.Items.Add(new ListItem(Localization.Translate("ItemCount"), "**count**"));
